...
public class Partie{
	...
	boolean sacrifierCarte = false; // Oblige � sacrifier une carte
	boolean sacrifierGuide = false; // Oblige � sacrifier un guide
	
	boolean poserApocalypse = false; // Oblige � poser une carte apocalypse
	
	boolean stopSacrificeGuide = false; //Emp�che de sacrifier une carte Guide
	boolean stopApocalypse = false; // Emp�che de poser une carte apocalypse
	
	boolean PtsAction = false; //Autorise ou pas les points d'actions
	boolean effetDivinite = false; // Dit si un effet divinit� a d�j� �t� utilis� ou pas
	...
	// M�thode pour remplir le deck des cartes action et divinit�s
	public void creerDeck(){
		
		// On cr�e une liste de carte, qui sera la pioche des cartes actions
		collectionCarte = new ArrayList<Carte>();
		// On cr�e une liste de carte, qui sera la pioche des cartes divinit�s
		collectionCarteDivinite = new ArrayList<Carte>();
		//System.out.println("La m�thode a �t� lanc�e");
	...
	public void demarrerPartie(){
		...
		*************************************************************************************/
		...
		// Le premier � jouer lance le d�
		this.tourJoueur.get(0).lancerDe(this.listeJoueur);
		System.out.println("\n/// Tour : " +nbreTour+ " ///");
		System.out.println("/// Infos: "+this.tourJoueur.get(0).getNom()+ " lance le d� ///\n");
		if (this.listeJoueur.get(0).resultatDes>=0 && this.listeJoueur.get(0).resultatDes<1){
			System.out.println("\n Influence: Jour ! \n Les divinit�s d'origine Jour gagnent 2 points action  Jour (1)! \n Les divinit�s d'origine Aube gagnent 1 point d'action Jour (1)!");
		}
		else if (this.listeJoueur.get(0).resultatDes<=1 && this.listeJoueur.get(0).resultatDes<2){
			System.out.println("\n Influence: Nuit ! \n Les divinit�s d'origine Nuit gagnent 2 points action Nuit (2) ! \n Les divinit�s d'origine Cr�puscule gagnent 1 point d'action Nuit (2)  !");
		}
		else{
		...
		for (int j=0; j<tourJoueur.size();j++){
			if (this.tourJoueur.get(j)== this.listeJoueur.get(0)){
				...
				System.out.println("\n Que voulez-vous faire ?\n - Jouer une carte (1)\n - Sacrifier une carte (2)\n - Utiliser effet divinit� (3)\n - Passer mon tour (4)");
				nbre = sc.nextInt();
				
				switch (nbre){
				case 1:  // Si on choisi 1, on veut jouer une carte
					if (sacrifierCarte==false && sacrifierGuide==false){ // On peut le faire si: on est pas oblig� de sacrifier, ni de sacrifier un guide
						System.out.println("Quelle carte voulez-vous jouer ?");
						rep = sc.nextInt();
						if ((this.listeJoueur.get(0).main.get(rep).getIdentifiantCarte()>=1 && this.listeJoueur.get(0).main.get(rep).getIdentifiantCarte()<=37) && poserApocalypse==false){ // Si on est pas oblig� de poser une apocalypse, et on a 1 pts d'action d'origine que la carte
							if (this.listeJoueur.get(0).nombrePointAction[this.listeJoueur.get(0).main.get(rep).origineCarte] !=0){ // Si on a des points correspondants � l'origine de la carte, on peut la jouer
							...
							else if(this.listeJoueur.get(0).main.get(rep).origineCarte ==3){ // Si on a une carte d'origine N�ant
						...
						else if ((this.listeJoueur.get(0).main.get(rep).getIdentifiantCarte()>=38 && this.listeJoueur.get(0).main.get(rep).getIdentifiantCarte()<=57) && poserApocalypse==false){ // Si on est pas oblig� de poser une apocalypse, et on a 1 pts d'action d'origine que la carte
							if (this.listeJoueur.get(0).nombrePointAction[this.listeJoueur.get(0).main.get(rep).origineCarte] !=0){ // Si on a des points correspondants � l'origine de la carte, on peut la jouer
								this.listeJoueur.get(0).setPointAction(this.listeJoueur.get(0).main.get(rep).origineCarte,-1);
								this.listeJoueur.get(0).poserCarte(rep, this.cartePlateau, this.getCollectionCarte());
								choixJoueur = true;
						...
						else if((this.listeJoueur.get(0).main.get(rep).getIdentifiantCarte()>=58 && this.listeJoueur.get(0).main.get(rep).getIdentifiantCarte()<=75) && poserApocalypse==false){ // Si c'est une carte Deus Ex et on est pas oblig� de poser une apocalypse
							if (this.listeJoueur.get(0).main.get(rep).origineCarte ==0){
								this.listeJoueur.get(0).poserCarte(rep, this.cartePlateau, this.getCollectionCarte());
								choixJoueur = true;
							}
							else if (this.listeJoueur.get(0).nombrePointAction[this.listeJoueur.get(0).main.get(rep).origineCarte] !=0){ // Si on a des points correspondants � l'origine de la carte, on peut la jouer
								this.listeJoueur.get(0).setPointAction(this.listeJoueur.get(0).main.get(rep).origineCarte,-1);
								this.listeJoueur.get(0).poserCarte(rep, this.cartePlateau, this.getCollectionCarte());
								choixJoueur = true;
				...
				case 2: // Si on choisi 2, on veut sacrifier une carte
					if (poserApocalypse == false){ // Si on est pas oblig� de poser une carte apocalypse, /!\ PAS OBLIGER D'UTILISER EFFET DIVINITE /!\
				...
				case 3: // Si on choisi 3, on veut utiliser l'effet divinit�
					...
					break;
				case 4: // On d�cide de sauter son tour
			...
			// On passe au joueur suivant
				...
				// On fait jouer les autres joueurs avec leur strat�gie
			}
	...
	// M�thode pour m�langer les cartes action et divinit�s
	public void melangerDeck(){
	Collections.shuffle(collectionCarte); // On m�lange la liste des cartes action du deck
	Collections.shuffle(collectionCarteDivinite); // On m�lange la liste des cartes divinit�s du deck
	}
	
	// M�thode pour cr�er la liste des joueurs
	public void creerListeJoueur( int nombreJoueur){
		listeJoueur = new ArrayList<Joueur>();
		tourJoueur = new ArrayList<Joueur>();
		for (int i=0; i<nombreJoueur; i++)
		{
		player = new Joueur(); // On cr�e le joueur num�ro i
	...
	// Methode pour distribuer carte Divinit�
	public void distribuerDivinite(){
		 for (int i=0; i<listeJoueur.size();i++){
			 listeJoueur.get(i).setDivinite(collectionCarteDivinite.get(0)); // On attribut au joueur sa carte divinit�
			 collectionCarteDivinite.remove(0); // On enl�ve alors cette carte de la liste des cartes Divinit�s
		}	
	}
	
	// Methode pour distribuer cartes
	public void distribuerCarte(){
		for (int i=0; i<listeJoueur.size(); i++){ // On r�p�te l'op�ration pour plusieurs chaque joueur de la liste
			int nombre = listeJoueur.get(i).getMain().size();
			//int nombre = listeJoueur.get(i).getMain().size(); // On met dans une variable le nombre de carte que poss�de le joueur i 
			while(nombre < 7){ // Tant que le joueur n'a pas 7 cartes, on r�p�te l'op�ration
				
				
				listeJoueur.get(i).getMain().add(this.collectionCarte.get(0)); // On donne cette carte au joueur i
				collectionCarte.remove(0); // On enl�ve cette carte de la liste
				nombre++;	
			}
			// Pour chaque joueur, on prend la carte 1 de la collection, on la stock dans carte 0, et on ajoute la carte 0 dans la collection main , on r�p�te �a tant que la taille de collection main <=7 fois 
		}
	}
